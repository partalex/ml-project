...
DATA_PATH: Path = Path("../res/data/bank-additional/bank-additional-full.csv")
DATA_SMALL: Path = Path("../res/data/bank/bank-full.csv")
OUT_PATH: str = "../out"


def load_data(path: Path) -> DataFrame:
    df: DataFrame = read_csv(path, sep=";")
    return df
...
def split_features_target(df: DataFrame) -> tuple[DataFrame, Series]:
    ...
    Returns:
        tuple: A tuple containing two elements:
            ...
            - labels (Series): A Series containing the target variable 'y' mapped to binary values (0 for 'no' and 1 for 'yes').
    """
    features: DataFrame = df.drop(columns=["y"])
    labels: Series = df["y"].map({"no": 0, "yes": 1})
